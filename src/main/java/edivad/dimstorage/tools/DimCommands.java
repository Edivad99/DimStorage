    ...
    public static void init(CommandDispatcher<CommandSourceStack> dispatcher) {
        root.then(Commands.literal("add").requires(cs -> cs.hasPermission(0)).executes(context -> {
            ...
            if(Config.DimTablet.addItem(item))
                player.displayClientMessage(Component.literal("Item added to the list").withStyle(ChatFormatting.GREEN), false);
            else
                player.displayClientMessage(Component.literal("You must select a valid item").withStyle(ChatFormatting.RED), false);
            return 0;
        }));
        ...
        root.then(Commands.literal("remove").requires(cs -> cs.hasPermission(0)).executes(context -> {
            ...
            if(Config.DimTablet.removeItem(item))
                player.displayClientMessage(Component.literal("Item removed from the list").withStyle(ChatFormatting.GREEN), false);
            else
                player.displayClientMessage(Component.literal("You must select a valid item").withStyle(ChatFormatting.RED), false);
            return 0;
        }));
        ...
        root.then(Commands.literal("removeAll").requires(cs -> cs.hasPermission(0)).executes(context -> {
            ...
            var player = context.getSource().getPlayerOrException();
            player.displayClientMessage(Component.literal("Removed all items from the list").withStyle(ChatFormatting.GREEN), false);
            return 0;
        }));

        root.then(Commands.literal("list").requires(cs -> cs.hasPermission(0)).executes(context -> {
            var player = context.getSource().getPlayerOrException();
            player.displayClientMessage(Component.literal("These are the items that the DimTablet will move:"), false);
